...
class EntreeTourelles(Entree):
    def __init__(self, v1, v2, v3, t1, t2, t3, start, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.data[self.hote].update(v1=v1, v2=v2, v3=v3, t1=t1 % (2 * pi), t2=t2 % (2 * pi), t3=t3 % (2 * pi), start=start)
        self.printe(self.data)

    def process(self, v1, v2, v3, t1, t2, t3, start, **kwargs):
        r = input('> ')
        if 'é' in r: v1 += 5
        if 'p' in r: v2 += 5
        if 'o' in r: v3 += 5
        ...
        if ',' in r: start = False
        self.data[self.hote].update(v1=v1, v2=v2, v3=v3, t1=round(t1 % (2 * pi), 1), t2=round(t2 % (2 * pi), 1), t3=round(t3 % (2 * pi), 1), start=start)
...
entree_tourelles_parser.add_argument('-s', '--start', action='store_true', help="starte la puissance")
